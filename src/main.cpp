...
#if defined(NDEBUG)
#error "XUMA cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uXMX) are considered zero fee (for relaying and mining)
...
const string strMessageMagic = "XUMA Signed Message:\n";
...
{
    //Check to see if the zXMX is properly signed
    if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
        if (!spend.HasValidSignature())
            return error("%s: V2 zXMX spend does not have a valid signature", __func__);

        libzerocoin::SpendType expectedType = libzerocoin::SpendType::SPEND;
        if (tx.IsCoinStake())
            expectedType = libzerocoin::SpendType::STAKE;
        if (spend.getSpendType() != expectedType) {
            return error("%s: trying to spend zXMX without the correct spend type. txid=%s", __func__,
                         tx.GetHash().GetHex());
        }
    }
    ...
    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
        return error("%s : zXMX spend with serial %s is already in block %d\n", __func__,
    ...
    if (pindex->nHeight > Params().Zerocoin_Block_EnforceSerialRange() &&
        ...
        return error("%s : zXMX spend with serial %s from tx %s is not in valid range\n", __func__,
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zXMX spend tx %s already in block %d",
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!ContextualCheckZerocoinSpend(tx, spend, chainActive.Tip(), 0))
                    return state.Invalid(error("%s: ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                               tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zXMX");
            }
        } else {
            LOCK(pool.cs);
            ...
            // Check that zXMX mints are not already known
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZXMXStake)
{
	if (Params().NetworkID() == CBaseChainParams::TESTNET) {
...
bool IsInitialBlockDownload()
{
    ...
    if (fImporting || fReindex || fVerifyingBlocks || chainActive.Height() < Checkpoints::GetTotalBlocksEstimate()){
        LogPrintf("Info: still initial Blockdownload: %i\n",chainActive.Height());
        return true;
    }
...
void static InvalidBlockFound(CBlockIndex* pindex, const CValidationState& state)
{
    ...
    if (state.IsInvalid(nDoS)) {
        ...
        if (it != mapBlockSource.end() && State(it->second)) {
            ...
            if (nDoS > 0){
                LogPrintf("Error: Invalid Block Found nDos bigger 0\n");
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from XUMA
...
{
    RenameThread("xuma-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZXMXMinted()
{
...
void RecalculateZXMXSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zXMX supply
...
bool RecalculateXMXSupply(int nHeightStart)
{
...
bool UpdateZXMXSupply(const CBlock& block, CBlockIndex* pindex)
{
    std::list<CZerocoinMint> listMints;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
        ...
        RecalculateXMXSupply(Params().Zerocoin_StartHeight());
    }

    //Track zXMX money supply in the block index
    if (!UpdateZXMXSupply(block, pindex))
        return state.DoS(100, error("%s: Failed to calculate new zXMX supply for block=%s height=%d", __func__,
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zXmxSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert XMX to zXMX
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zXMX spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn& txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zXMX serial %s in block\n Block: %s",
...
bool CheckWork(const CBlock block, CBlockIndex* const pindexPrev)
{
    ...
    if (block.IsProofOfWork() && (pindexPrev->nHeight + 1 <= 299999)) {
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
    if (nHeight < Params().Zerocoin_Block_V2_Start())
        return error("%s: zXMX stake block is less than allowed start height", __func__);

    if (CZXmxStake* zXMX = dynamic_cast<CZXmxStake*>(stake)) {
        CBlockIndex* pindexFrom = zXMX->GetIndexFrom();
        if (!pindexFrom)
            return error("%s: failed to get index associated with zXMX stake checksum", __func__);

        if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
            return error("%s: zXMX stake does not have required confirmation depth", __func__);
        ...
        uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zXMX->GetValue()));
        if (nChecksum200 != zXMX->GetChecksum())
            return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zXMX->GetChecksum(), nChecksum200);
    } else {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        if (stake->IsZXMX() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
            return state.DoS(100, error("%s: staked zXMX fails context checks", __func__));
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zXMX mints and %d zXMX spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        if (pfrom->nVersion != 0) {
            ...
            LOCK(cs_main);
            LogPrintf("Error: duplicate Version Message\n");
            Misbehaving(pfrom->GetId(), 1);
            return false;
        }

        // XUMA: We use certain sporks during IBD, so check to see if they are
        ...
        if (vAddr.size() > 1000) {
            LOCK(cs_main);
            LogPrintf("Error: vAddr size bigger 1000\n");
        ...
        if (vInv.size() > MAX_INV_SZ) {
            LOCK(cs_main);
            LogPrintf("Error vInv Size bigger than MAX INV SIZE\n");
            Misbehaving(pfrom->GetId(), 20);
        ...
        for (unsigned int nInv = 0; nInv < vInv.size(); nInv++) {
            ...
            if (pfrom->nSendSize > (SendBufferSize() * 2)) {
                LogPrintf("Error: wrong sendbuffer\n");
        ...
        if (vInv.size() > MAX_INV_SZ) {
            LOCK(cs_main);
            LogPrintf("Error: getdata size bigger than MAX INV SIZE\n");
            Misbehaving(pfrom->GetId(), 20);
                ...
                for(set<uint256>::iterator mi = itByPrev->second.begin();
                    ...
                    if(AcceptToMemoryPool(mempool, stateDummy, orphanTx, true, &fMissingInputs2)) {
                        ...
                        if(stateDummy.IsInvalid(nDos) && nDos > 0) {
                            ...
                            LogPrintf("Error: got orphan Hash from Peer!\n");
        ...
        if (state.IsInvalid(nDoS)) {
            ...
            if (nDoS > 0){
                LogPrintf("Error: reject Message mempool\n");
    ...
    else if (strCommand == "headers" && Params().HeadersFirstSyncingActive() && !fImporting && !fReindex) // Ignore headers received while importing
    {
        ...
        if (nCount > MAX_HEADERS_RESULTS) {
            LOCK(cs_main);
            LogPrintf("Error: headers message size bigger than MAX HEADERS RESULTS\n");
            Misbehaving(pfrom->GetId(), 20);
        ...
        BOOST_FOREACH (const CBlockHeader& header, headers) {
            ...
            if (pindexLast != NULL && header.hashPrevBlock != pindexLast->GetBlockHash()) {
                LogPrintf("Error: non-continuous headers sequence\n");
                Misbehaving(pfrom->GetId(), 20);
            ...
            if (!AcceptBlockHeader((CBlock)header, state, &pindexLast)) {
                ...
                if (state.IsInvalid(nDoS)) {
                    if (nDoS > 0){
                        LogPrintf("Error: Accept Block Header: State is invalid\n");
                        Misbehaving(pfrom->GetId(), nDoS);
                    }
        ...
        if (!mapBlockIndex.count(block.hashPrevBlock)) {
            ...
            if (!mapBlockIndex.count(block.GetHash())) {
                ...
                if(state.IsInvalid(nDoS)) {
                    ...
                    if(nDoS > 0) {
                        TRY_LOCK(cs_main, lockMain);
                        LogPrintf("Error: AddInventory: State is invalid\n");
    ...
    // getaddr message mitigates the attack.
        ...
        if (pfrom->setKnown.count(alertHash) == 0) {
            if (alert.ProcessAlert()) {
                ...
                LOCK(cs_main);
                LogPrintf("Error: invalid signature received\n");
        ...
        LOCK(cs_main);
        LogPrintf("Error: filter bloom error\n");
        ...
        if (!filter.IsWithinSizeConstraints()) {
            ...
            LOCK(cs_main);
            LogPrintf("Error: filter is not within size constraints\n");
            Misbehaving(pfrom->GetId(), 100);
        } else {
            LOCK(pfrom->cs_filter);
        ...
        if (vData.size() > MAX_SCRIPT_ELEMENT_SIZE) {
            LOCK(cs_main);
            LogPrintf("Error: filteradd size bigger than MAX SCRIPT ELEMENT SIZE\n");
            Misbehaving(pfrom->GetId(), 100);
        } else {
            LOCK(pfrom->cs_filter);
            if (pfrom->pfilter)
