...
void FillBlockPayee(CMutableTransaction& txNew, CAmount nFees, bool fProofOfStake, bool fZXMXStake)
{
    CBlockIndex* pindexPrev = chainActive.Tip();
    ...
    if (IsSporkActive(SPORK_13_ENABLE_SUPERBLOCKS) && budget.IsBudgetPaymentBlock(pindexPrev->nHeight + 1)) {
        ...
        masternodePayments.FillBlockPayee(txNew, nFees, fProofOfStake, fZXMXStake);
    }
}
...
void CMasternodePayments::FillBlockPayee(CMutableTransaction& txNew, int64_t nFees, bool fProofOfStake, bool fZXMXStake)
{
    CBlockIndex* pindexPrev = chainActive.Tip();
    if (!pindexPrev) return;
	...
	BOOST_FOREACH (CTxOut out, txNew.vout) {
		if(out.scriptPubKey == developerfeescriptpubkey) {
			if(out.nValue >= requiredDeveloperPayment) {
				...
				LogPrint("masternode", "Developer-Fee Payment found! Thanks for supporting XUMA!");
			}
		}
	}
