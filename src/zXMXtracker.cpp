...
CzXMXTracker::CzXMXTracker(std::string strWalletFile)
{
...
CzXMXTracker::~CzXMXTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzXMXTracker::Init()
{
...
bool CzXMXTracker::Archive(CMintMeta& meta)
{
...
bool CzXMXTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzXMXTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzXMXTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzXMXTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzXMXTracker::GetSerialHashes()
{
...
CAmount CzXMXTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzXMXTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzXMXTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzXMXTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzXMXTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzXMXTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzXMXTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzXMXTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzXMXTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzXMXTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzXMXTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzXMXTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzXMXTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzXMXTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzXMXTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzXMXTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzXMXTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzXMX from DB\n", __func__, listDeterministicDB.size());
    }
