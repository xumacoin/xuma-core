...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZXmxStake(spend));
    } else {
        ...
        CXMXStake* XMXInput = new CXMXStake();
        XMXInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(XMXInput);
    }
