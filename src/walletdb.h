...
public:
    ...
    bool WriteZXMXSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZXMXSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZXMXSeed_deprecated(uint256& seed);
    bool EraseZXMXSeed();
    bool EraseZXMXSeed_deprecated();

    bool WriteZXMXCount(const uint32_t& nCount);
    bool ReadZXMXCount(uint32_t& nCount);
