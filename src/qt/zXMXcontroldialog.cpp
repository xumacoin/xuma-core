...
#include "ui_zXMXcontroldialog.h"
...
std::set<std::string> ZXmxControlDialog::setSelectedMints;
std::set<CMintMeta> ZXmxControlDialog::setMints;

bool CZXmxControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZXmxControlDialog::COLUMN_DENOMINATION || column == ZXmxControlDialog::COLUMN_VERSION || column == ZXmxControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}

ZXmxControlDialog::ZXmxControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZXmxControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZXmxControlDialog::~ZXmxControlDialog()
{
    delete ui;
}

void ZXmxControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZXmxControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZXmxControlWidgetItem* itemDenom(new CZXmxControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZXmxControlWidgetItem *itemMint = new CZXmxControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
...
void ZXmxControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZXmxControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZXmx_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZXmxControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZXmxControlDialog::GetSelectedMints()
{
...
void ZXmxControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
