...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered xuma address / amount:
...

    {
        ...
        if (recipients[0].useHyperSend && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("HyperSend doesn't support sending values that high yet. Transactions are currently limited to %1 XMX.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal xuma:URI (xuma:XyZ...?message=example)
            {
...
static void NotifyzXMXReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzXMXReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzXMXReset.connect(boost::bind(NotifyzXMXReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzXMXReset.disconnect(boost::bind(NotifyzXMXReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zXMXTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
