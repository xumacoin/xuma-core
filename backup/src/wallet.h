...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 XMX at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zXMX send
enum ZerocoinSpendStatus {
    ZXMX_SPEND_OKAY = 0,                            // No error
    ZXMX_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZXMX_WALLET_LOCKED = 2,                         // Wallet was locked
    ZXMX_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZXMX_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZXMX_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZXMX_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZXMX_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZXMX_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZXMX_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZXMX_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZXMX_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZXMX_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZXMX_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZXMX_SPENT_USED_ZXMX = 14,                      // Coin has already been spend
    ZXMX_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZXMX_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZXmxBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZXMXOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzXMXAuto) const;
    ...
    */
    ...
    CzXMXWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzXMXTracker> zXMXTracker;
    ...
    void setZWallet(CzXMXWallet* zwallet)
    {
        zwalletMain = zwallet;
        zXMXTracker = std::unique_ptr<CzXMXTracker>(new CzXMXTracker(strWalletFile));
    }

    CzXMXWallet* getZWallet() { return zwalletMain; }
    ...
    void setZXmxAutoBackups(bool fEnabled)
    {
    ...
    /** zXMX reset */
    boost::signals2::signal<void()> NotifyzXMXReset;
