...
CzXMXWallet::CzXMXWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zXMX seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZXMXSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZXMXSeed_deprecated()) {
                    LogPrintf("%s: Updated zXMX seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zXMX seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zXMX wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzXMXWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZXMXCount(nCountLastUsed);
    else if (!walletdb.ReadZXMXCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzXMXWallet::Lock()
{
    seedMaster = 0;
}

void CzXMXWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzXMXWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZXMX(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzXMXWallet::LoadMintPoolFromDB()
{
...
void CzXMXWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzXMXWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzXMXWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zXMXTracker->HasPubcoinHash(pMint.first)) {
...
bool CzXMXWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZXMX(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zXMXTracker which also adds to database
    pwalletMain->zXMXTracker->Add(dMint, true);
...
void CzXMXWallet::SeedToZXMX(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzXMXWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzXMXWallet::GenerateDeterministicZXMX(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzXMXWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzXMXWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
